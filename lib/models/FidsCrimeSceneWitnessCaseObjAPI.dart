...
class FidsCrimeSceneWitnessCaseObjAPI {
  String? fidsNo;
  String? departmentId;
  ...
  FidsCrimeSceneWitnessCaseObjAPI(
      {this.fidsNo,
      this.departmentId,
      this.casecategoryId,
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'exhibit_time': exhibitTime == 'null' ? '' : exhibitTime,
    };
  }

  Future<List<FidsCrimeSceneWitnessCaseObjAPI>> generateModel(
      FidsCrimeScene data) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var uGroup = prefs.getString('uGroup');

    FidsCrimeSceneWitnessCaseObjAPI response =
        FidsCrimeSceneWitnessCaseObjAPI();
    ...
    var resultCaseExhibit =
        await CaseExhibitDao().getCaseExhibit(data.fidsid ?? -1);
    response.caseExnibit = resultCaseExhibit;
    ...
    response.objective = data.objective;
    response.exhibitLocation = data.exhibitLocation;
    response.exhibitDate = data.exhibitDate;
    response.exhibitTime = data.exhibitTime;

    List<FidsCrimeSceneWitnessCaseObjAPI> go = [];
    go.add(response);
    return go;
  }
}
