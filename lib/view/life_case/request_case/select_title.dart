...
class SelectTitle extends StatefulWidget {
  const SelectTitle({super.key});

  @override
  SelectTitleState createState() => SelectTitleState();
}

class SelectTitleState extends State<SelectTitle> {
  bool isPhone = Device.get().isPhone;
  List<MyTitle> data = [];
  List<MyTitle> masterData = [];
      ...
      child: InputFieldWithIcon(
        ...
        onChanged: (value) {
          if (value.isEmpty || value == '') {
            setState(() {
              data = List<MyTitle>.from(masterData);
            });
            return;
          }
  ...
  void asyncCall() async {
    var result = await TitleDao().getTitle();
    setState(() {
      masterData = result;
      data = List<MyTitle>.from(masterData);
    });
