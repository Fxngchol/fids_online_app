...
class SelectVehicleType extends StatefulWidget {
  ...
  @override
  SelectVehicleTypeState createState() => SelectVehicleTypeState();
}

class SelectVehicleTypeState extends State<SelectVehicleType> {
  bool isPhone = Device.get().isPhone;
  List<VehicleType> data = [];
  List<VehicleType> masterData = [];
      ...
      child: InputFieldWithIcon(
        ...
        onChanged: (value) {
          if (value.isEmpty || value == '') {
            setState(() {
              data = List<VehicleType>.from(masterData);
            });
            return;
          }
          data.clear();
          for (int i = 0; i < masterData.length; i++) {
            if ('${masterData[i].vehicleType}'.contains('$value'.trim())) {
  ...
  void asyncCall() async {
    var result = await VehicleTypeDao().getVehicleTypeList();
    setState(() {
      masterData = result;
      data = List<VehicleType>.from(masterData);
    });
